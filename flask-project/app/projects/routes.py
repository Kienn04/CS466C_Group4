...
def move_task(project_id: int, task_id: int):
    ...
    current_assignee_ids = {user.id for user in task.assignees}
    if not is_owner and task.created_by_id != current_user.id and current_user.id not in current_assignee_ids:
        abort(403)
